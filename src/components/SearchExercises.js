...
const SearchExercises = ({ setExercises, bodyPart, setBodyPart }) => {
  ...
  return (
    ...
    (item.bodyPart && item.bodyPart.toLowerCase().includes(searchTerm)) ||
    ...
    (Array.isArray(item.secondaryMuscles) &&
      item.secondaryMuscles.some((muscle) => muscle.toLowerCase().includes(searchTerm))) ||
    (Array.isArray(item.instructions) &&
      item.instructions.some((inst) => inst.toLowerCase().includes(searchTerm)))
  );
});
      ...
      setSearch('');
      console.log("Searched exercise - ",searchedExercises);
